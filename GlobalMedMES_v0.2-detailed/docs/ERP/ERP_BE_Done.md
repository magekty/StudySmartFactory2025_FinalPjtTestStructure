...
간단한 다이어그램 아래와 같은 내용을 기반으로 그려줘

생산계획 (조회-계획코드기반상태와날짜를검색하면조회된내용확인가능20행마다새페이지,신규-항상 폼을 초기화,생성-계획코드+품목+기간+수량+비고 내용 등록 계획코드 미입력시 생성버튼비활성화,수정-조회후테이블에서선택시신규폼에해당계획기반데이터자동입력및계획코드기반존재시수정으로반영,확정-draft시확정버튼활성화,취소-confirmed및draft상태에서만취소버튼활성화,삭제-소프트삭제DB삭제컬럼으로운영confirmed및draft및canceled만 삭제버튼 활성화, 품목선택시-품목조회모달확장품번,품명기준검색가능및조회된테이블품번품명규격단위상태표시)