...
CREATE TABLE IF NOT EXISTS mes_outbox (
  outbox_id      BIGINT NOT NULL AUTO_INCREMENT COMMENT 'PK',
  event_type     VARCHAR(50)  NOT NULL COMMENT 'WO_CREATE/WO_STATUS/PERF_CREATE/BACKFLUSH/COST_POST',
  payload_json   LONGTEXT      NOT NULL COMMENT '전송 페이로드(JSON)',
  idempotency_key VARCHAR(64)  NULL COMMENT '멱등 헤더 키(없으면 워커가 생성)',
  status         VARCHAR(20)  NOT NULL COMMENT 'PENDING/SENT/RETRY/FAILED/SHADOWED',
  retry_count    INT           NOT NULL DEFAULT 0,
  next_retry_at  DATETIME      NULL COMMENT '다음 재시도 시각(UTC)',
  created_at     DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성(UTC)',
  last_error     VARCHAR(500)  NULL,

  PRIMARY KEY (outbox_id),
  KEY idx_outbox_pick (status, next_retry_at, created_at),
  KEY idx_outbox_created (created_at),
  UNIQUE KEY uk_outbox_idem (idempotency_key)
) ENGINE=InnoDB COMMENT='ERP 연동 Outbox 큐';

CREATE TABLE IF NOT EXISTS tb_sync_cursor (
  cursor_key VARCHAR(50) PRIMARY KEY,
  last_synced_at DATETIME NOT NULL
) ENGINE=InnoDB COMMENT='증분 동기화 커서';

-- BOM 헤더
CREATE TABLE IF NOT EXISTS `tb_bom_header` (
  `bom_id`      VARCHAR(50)  NOT NULL COMMENT 'BOM ID (PK)',
  `item_id`     VARCHAR(36)  NOT NULL COMMENT '모품목 ID (FK → tb_item.item_id)',
  `revision`    VARCHAR(20)  NOT NULL COMMENT '리비전',
  `eff_from`    DATETIME     NOT NULL COMMENT '유효 시작(UTC)',
  `eff_to`      DATETIME     DEFAULT NULL COMMENT '유효 종료(UTC, NULL=현재)',
  ...
  KEY `idx_bom_rev_eff` (`revision`, `eff_from`),
  CONSTRAINT `fk_bom_item` FOREIGN KEY (`item_id`) REFERENCES `tb_item`(`item_id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='BOM 헤더: 유효기간/리비전 포함(소스=ERP, MES 캐시)';

-- BOM 라인
CREATE TABLE IF NOT EXISTS `tb_bom_line` (
  `line_id`      BIGINT       NOT NULL AUTO_INCREMENT COMMENT '라인 ID (PK)',
  `bom_id`       VARCHAR(50)  NOT NULL COMMENT 'BOM ID (FK → tb_bom_header.bom_id)',
  `component_id` VARCHAR(36)  NOT NULL COMMENT '구성품 품목 ID (FK → tb_item.item_id)',
  `qty`          DECIMAL(18,6) NOT NULL DEFAULT 0 COMMENT '표준 소요 수량',
  `uom`          VARCHAR(20)  NOT NULL COMMENT '단위(EA, KG 등)',
  `scrap_rate`   DECIMAL(9,6)  NOT NULL DEFAULT 0 COMMENT '스크랩율(0~1)',
  `is_deleted`   TINYINT       DEFAULT 0 COMMENT '소프트삭제',
  `deleted_at`   DATETIME      DEFAULT NULL COMMENT 'UTC',
  `created_by`   VARCHAR(50)   NOT NULL DEFAULT 'sync',
  `created_at`   DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'UTC',
  ...
  KEY `idx_bomline_comp` (`component_id`),
  UNIQUE KEY `uk_bom_comp` (`bom_id`, `component_id`),
  CONSTRAINT `fk_bomline_bom`  FOREIGN KEY (`bom_id`)       REFERENCES `tb_bom_header`(`bom_id`)  ON DELETE CASCADE,
  CONSTRAINT `fk_bomline_item` FOREIGN KEY (`component_id`) REFERENCES `tb_item`(`item_id`)       ON DELETE RESTRICT,
