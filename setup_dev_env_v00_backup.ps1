Param(
  ...
  [string]$SeedFile = "tools\sql\dev_seed_plus_ordered.sql", # 있으면 주입, 없으면 건너뜀
  [switch]$AutoPort,           # 포트 점유 시 자동으로 다음 빈 포트 탐색
  [switch]$StrictVersion,      # Java<17 또는 Node<18이면 즉시 중단
  [switch]$NonInteractive      # 사용자 입력 프롬프트 없이 오류 시 중단
)
...
function Require-Cmd($name) {
  if (-not (Get-Command $name -ErrorAction SilentlyContinue)) { Fail "$name 필요"; throw "$name not found"; }
}

# 포트 사용 여부 체크 (관리자 권한이 아닐 때 Get-NetTCPConnection 실패 대비)
function Test-PortUse {
  ...
  try {
    ...
    # 관리자 권한이 아니거나 Get-NetTCPConnection 실패 시 netstat 대체
...
function Find-FreePort {
  ...
  $port = $Start
  for ($i=0; $i -lt 100; $i++) {
    $t = Test-PortUse -Port $port
    if (-not $t.Used) { return $port }
    $port++
  }
  return $null
}
...
function Remove-ContainerIfExists {
  ...
  if ($names) {
    ...
    $names -split "\r?\n" | ForEach-Object {
      if ($_ -ne "") { docker rm -f $_ | Out-Null }
    }
  }
}
...
# Java/Node 버전 확인
# --- Java/Node 버전 체크(에러 안 나는 안전 버전) ---
$javaOk = $true; $nodeOk = $true

# Java: stderr 출력이라도 오류 레코드 안 만들도록 cmd 래퍼 사용
try {
  $javaOut = cmd /c "java -version 2>&1"
  if (-not $javaOut) { $javaOk = $false; Warn "Java 실행 실패 또는 미설치" }
  else {
    # 예: openjdk version "17.0.16" 2025-07-15
    ...
    if (-not $javaVer) { $javaOk = $false; Warn "Java 버전 파싱 실패: $javaOut" }
      ...
      if ($javaMajor -lt 17) { $javaOk = $false; Warn "Java 17 권장(현재: $javaVer)" }
    }
  }
} catch {
  $javaOk = $false; Warn "Java 확인 중 예외: $($_.Exception.Message)"
}

# Node: v18 이상 권장
try {
  $nodeRaw = (& node -v) 2>$null  # v18.20.3 형태
  if (-not $nodeRaw) { $nodeOk = $false; Warn "Node 미설치 또는 PATH 미등록" }
  else {
    $nodeVer = $nodeRaw.TrimStart('v')
    $nodeMajor = [int]($nodeVer.Split('.')[0])
    if ($nodeMajor -lt 18) { $nodeOk = $false; Warn "Node 18 권장(현재: v$nodeVer)" }
  }
} catch {
  $nodeOk = $false; Warn "Node 확인 중 예외: $($_.Exception.Message)"
}

# 기준 미달 처리(Strict/Interactive 정책 유지)
if (-not $javaOk -or -not $nodeOk) {
  ...
  if (-not $NonInteractive) {
    ...
    Warn "NonInteractive: 버전 경고 무시하고 계속"
  }
}
# --- 끝 ---
# ---------- 2) 포트 점유 체크 ----------
...
$portInfo = $null
try {
  $conns = Get-NetTCPConnection -LocalPort $MysqlPort -ErrorAction Stop
  if ($conns) {
    $own = $conns | Select-Object -First 1
    $procId   = $own.OwningProcess
    $procName = (Get-Process -Id $procId -ErrorAction SilentlyContinue).ProcessName
    $portInfo = @{ Used = $true; Detail = @{ PID=$procId; Name=$procName } }
  }
} catch {
  $raw = netstat -ano | Select-String -Pattern "LISTENING.*[:.]$MysqlPort\s"
  if ($raw) {
    $pId = ($raw -split "\s+")[-1]
    $procName = (Get-Process -Id $pId -ErrorAction SilentlyContinue).ProcessName
    $portInfo = @{ Used = $true; Detail = @{ PID=$pId; Name=$procName } }
  } else { $portInfo = @{ Used = $false; Detail = $null } }
}

if ($portInfo.Used) {
  $procId = $portInfo.Detail.PID; $procName = $portInfo.Detail.Name
  Write-Host "포트 $MysqlPort 점유중: PID=$procId, Name=$procName" -ForegroundColor Red
  if ($AutoPort) {
    $newPort = $MysqlPort + 1
    while ((Get-NetTCPConnection -LocalPort $newPort -ErrorAction SilentlyContinue)) { $newPort++ }
    Write-Host "대체 포트 자동 선택 → $newPort" -ForegroundColor Yellow
    $MysqlPort = $newPort
  } else {
    throw "포트 $MysqlPort 점유. -AutoPort 또는 -MysqlPort로 변경해 실행하세요."
  }
}
Ok "MySQL 포트 확정: $MysqlPort"

# ---------- 3) Backend/Frontend 경로 처리 ----------
...
try {
  ...
  if (Test-Path $SeedFile) {
    ...
    Warn "시드 파일 없음: $SeedFile → 주입 건너뜀 (개발만 먼저 띄울 수 있음)"
  }
  Ok "DB 초기화/시드 완료"
} catch {
  Fail "SQL 주입 중 오류: $($_.Exception.Message)"; exit 1
}

# ---------- 7) Backend/Frontend 환경 파일 및 기동 ----------
Info "==> [7/8] Backend/Frontend 환경 반영 및 기동"

# Backend application-local.yml
$appLocal = Join-Path $BackendDir "src\main\resources\application-local.yml"
$dirPath = Split-Path $appLocal
Ensure-Dir $dirPath
@"
server:
  port: ${ApiPort}
...
"@ | Out-File -Encoding UTF8 $feEnv
Ok "생성/갱신: $feEnv"

# 백/프 기동 (새 PowerShell 창)
if (Test-Path (Join-Path $BackendDir "gradlew")) {
  ...
  Warn "BackendDir에 gradlew 미검출 → 수동 기동"
}
...
Write-Host "API: http://localhost:${ApiPort}  (Spring 'local' 프로필)"
Write-Host "WEB: http://localhost:${WebPort}  (Vite dev server)"
Warn "[주의] 시드의 TB_USER.password_hash는 실제 bcrypt 해시 사용 권장"